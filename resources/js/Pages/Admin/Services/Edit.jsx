...
export default function Edit({ service }) {
    ...
    const [preview, setPreview] = useState(service.icon ? `/storage/${service.icon}` : null);

    useEffect(() => {
        if (!data.icon) {
            // If we cleared the icon selection, show the original one if it exists
            if (service.icon) {
                setPreview(`/storage/${service.icon}`);
            } else {
                setPreview(null);
            }
            return;
                                ...
                                <div className={`aspect-square rounded-3xl border-2 border-dashed border-white/10 flex flex-col items-center justify-center overflow-hidden transition-all group-hover:border-isak-primary/50 ${preview ? 'bg-black' : 'bg-white/2'}`}>
                                    {preview ? (
                                        ...
                                        <div className="text-center p-6">
                                            ...
                                            <p className="text-gray-500 text-sm font-medium">Click to change icon</p>
                                        </div>
                                    )}
                                    <input
                                        type="file"
